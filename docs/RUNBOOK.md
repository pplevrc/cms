...
# スクリプト経由 (一時ファイル + shred で消去)
gh secret set <NAME> < /tmp/secret && shred -u /tmp/secret
```
...
# スクリプト経由で値を渡す場合は一時ファイル経由 (使用後 shred で消す)
gh secret set CI_PAYLOAD_SECRET < /tmp/new-secret && shred -u /tmp/new-secret

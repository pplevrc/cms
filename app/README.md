...
`make help` で他の target (`dev` / `typecheck` / `lint` / `secretlint` / `test`) も確認できる。

`make setup` の中身を手で再現したい場合は次の各ステップを参照。
...
`PAYLOAD_SECRET` または `DATABASE_URL` が未設定 (空文字列を含む) の状態で `pnpm dev` を起動すると、`app/src/lib/env.ts` の `requireEnv` が「環境変数 X が設定されていません」エラーで明示的に落ちる。silent failure を避けるための意図的な挙動。
